<#
    MOCKUP 3: Landing/home screen with 3 big buttons leading into each section.
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        ...
        Title="Template Generator - Mockup 3 (Home Screen)" Height="560" Width="780"
        WindowStartupLocation="CenterScreen">
    <Grid Margin="10" Name="RootGrid">
        <!-- Home screen -->
        <StackPanel Name="HomePanel" VerticalAlignment="Center" HorizontalAlignment="Center">
            <TextBlock Text="Template Generator" FontSize="26" FontWeight="Bold" HorizontalAlignment="Center" Margin="0,0,0,30"/>
            <StackPanel Orientation="Horizontal" HorizontalAlignment="Center">
                <Button Name="Btn_Tickets" Content="Tickets" Width="150" Height="100" Margin="10" FontSize="16"/>
                <Button Name="Btn_Deferrals" Content="Deferrals" Width="150" Height="100" Margin="10" FontSize="16"/>
                <Button Name="Btn_Requests" Content="Requests" Width="150" Height="100" Margin="10" FontSize="16"/>
            </StackPanel>
        </StackPanel>

        <!-- Wizard host (hidden until a section is picked) -->
        <Grid Name="WizardHost" Visibility="Collapsed">
            <Grid.RowDefinitions>
                <RowDefinition Height="Auto"/>
                <RowDefinition Height="*"/>
            </Grid.RowDefinitions>
            <Button Name="HomeBtn" Content="&lt;&lt; Home" Width="90" HorizontalAlignment="Left" Margin="0,0,0,10" Grid.Row="0"/>
            <Grid Name="WizardContent" Grid.Row="1"/>
        </Grid>
    </Grid>
</Window>
"@
...
function New-WizardPanel {
    ...
    $stepLabel.FontWeight = "Bold"; $stepLabel.FontSize = 14; $stepLabel.Margin = "0,0,0,10"
...
$homePanel     = $window.FindName("HomePanel")
$wizardHost    = $window.FindName("WizardHost")
$wizardContent = $window.FindName("WizardContent")
$homeBtn       = $window.FindName("HomeBtn")

function Show-Section {
    param([string]$SectionName, [bool]$HasDeviceCountStep)
    $wizardContent.Children.Clear()
    $panel = New-WizardPanel -SectionName $SectionName -HasDeviceCountStep $HasDeviceCountStep
    $wizardContent.Children.Add($panel) | Out-Null
    $homePanel.Visibility = "Collapsed"
    $wizardHost.Visibility = "Visible"
}

$window.FindName("Btn_Tickets").Add_Click({ Show-Section -SectionName "Tickets" -HasDeviceCountStep $false })
$window.FindName("Btn_Deferrals").Add_Click({ Show-Section -SectionName "Deferrals" -HasDeviceCountStep $true })
$window.FindName("Btn_Requests").Add_Click({ Show-Section -SectionName "Requests" -HasDeviceCountStep $true })

$homeBtn.Add_Click({
    $wizardHost.Visibility = "Collapsed"
    $homePanel.Visibility = "Visible"
})
